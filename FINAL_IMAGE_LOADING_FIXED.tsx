...
export default class ImageUploadWebPart extends React.Component<IImageUploadWebPartProps, IState> {
  ...
  constructor(props: IImageUploadWebPartProps) {
    ...
    console.log('Web Absolute URL:', this.props.context.pageContext.web.absoluteUrl);
    console.log('Web Server Relative URL:', this.props.context.pageContext.web.serverRelativeUrl);
  ...
  private getLibraryPath(): string {
    // Get the full server relative path for API calls
    const webServerRelativeUrl = this.props.context.pageContext.web.serverRelativeUrl;
    const libraryPath = webServerRelativeUrl.endsWith('/') 
      ? webServerRelativeUrl + 'KaizenImages' 
      : webServerRelativeUrl + '/KaizenImages';
    console.log('Library path (API):', libraryPath);
    return libraryPath;
  }

  private getImageUrl(fileName: string): string {
    // Construct direct image URL (for <img src>)
    // Just use /sites/NXT-CBS/KaizenImages/filename
    const imageUrl = `${this.props.context.pageContext.web.absoluteUrl}/KaizenImages/${fileName}?t=${Date.now()}`;
      ...
      if (httpResponse.ok) {
        ...
        if (data.value && data.value.length > 0) {
          ...
          if (kaizenImages.length > 0) {
            ...
            const imageUrl: string = this.getImageUrl(file.Name);
